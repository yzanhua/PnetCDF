...
static bool resize0 (hashmap_t *map, size_t new_cap) {
    hashmap_t *map2 = hashmap_new_with_allocator (
        map->malloc, map->realloc, map->free, map->elsize, new_cap, map->seed0, map->seed1,
...
static bool resize (hashmap_t *map, size_t new_cap) { return resize0 (map, new_cap); }
...
const void *hashmap_get_with_hash (hashmap_t *map, const void *key, uint64_t hash) {
    ...
    while (1) {
        ...
        if (bucket->hash == hash) {
            ...
            if (!map->compare || map->compare (key, bitem, map->udata) == 0) { return bitem; }
        }
        i = (i + 1) & map->mask;
    }
}
...
bool hashmap_scan (hashmap_t *map, bool (*iter) (const void *item, void *udata), void *udata) {
    for (size_t i = 0; i < map->nbuckets; i++) {
        struct bucket *bucket = bucket_at (map, i);
        if (bucket->dib && !iter (bucket_item (bucket), udata)) { return false; }
    }
    return true;
}
